...
class DDQNAgent:
    ...
    def choose_action(self, cur_state):
        if np.random.uniform() < self.epsilon:
            ...
            #cur_state = cur_state.reshape(1,17,17)
    ...
    def learn(self):
        ...
        # cur_values = self.q_target.predict(states)
        # next_values = self.q_target.predict(new_states)
        #
        # batch_index = np.arange(self.batch_size, dtype=np.int32)
        # cur_values[batch_index, actions] =  rewards + self.gamma*np.amax(next_values, axis=1)*dones
        # self.q_target.fit(states, cur_values, epochs=1, verbose=0)
        ...
        # targets = rewards + self.gamma*np.amax(self.q_target.predict_on_batch(new_states), axis=1)*dones
        # targets_full = self.q_target.predict_on_batch(states)
        # batch_index = np.arange(self.batch_size, dtype=np.int32)
        # targets_full[batch_index, actions] = targets
        # _ = self.q_eval.fit(states, targets_full, epochs=1, verbose=0)
